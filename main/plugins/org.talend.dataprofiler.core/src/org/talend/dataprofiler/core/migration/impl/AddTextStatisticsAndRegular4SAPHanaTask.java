...
public class AddTextStatisticsAndRegular4SAPHanaTask extends AbstractWorksapceUpdateTask {
    ...
    private final String AVERAGE_LENGTH_WITH_BLANK_AND_NULL_BODY =
            "SELECT SUM(LENGTH(IFNULL(<%=__COLUMN_NAMES__%>,''))), COUNT(*) FROM <%=__TABLE_NAME__%> <%=__WHERE_CLAUSE__%>"; //$NON-NLS-1$

    private final String AVERAGE_LENGTH_WITH_BLANK_BODY =
            "SELECT SUM(LENGTH(<%=__COLUMN_NAMES__%>)), COUNT(*) FROM <%=__TABLE_NAME__%> WHERE (<%=__COLUMN_NAMES__%> IS NOT NULL ) <%=__AND_WHERE_CLAUSE__%>"; //$NON-NLS-1$
    ...
    private final String REGULAR_EXPRESSION_MATCHING_BODY =
            "SELECT COUNT(CASE WHEN <%=__COLUMN_NAMES__%> LIKE_REGEXPR <%=__PATTERN_EXPR__%> THEN 1 END), COUNT(*) FROM <%=__TABLE_NAME__%> <%=__WHERE_CLAUSE__%>"; //$NON-NLS-1$

    private final String language = DbmsLanguage.SAPHANA;

    private DefinitionHandler definitionHandler;

    @Override
    public Date getOrder() {
        return createDate(2020, 12, 2);
    }
